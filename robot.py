...
# List to store top 10 Gbest positions each iteration
top_10_gbest_positions = []
...
def save_top_10_gbest(iteration, top_10):
    """Save top 10 Gbest positions to a text file"""
    with open(f"top_10_gbest_iteration_{iteration}.txt", "w") as f:
        for i, pos in enumerate(top_10):
            f.write(f"Position {i+1}:\n")
            np.savetxt(f, pos.reshape(12, 10), fmt='%.6f')  # Reshape for readability
            f.write("\n")

def load_random_top_10():
    """Load random 10 positions from the saved top 10 Gbest positions"""
    if not top_10_gbest_positions:
        return None
    
    # Select 10 random positions from all saved top positions
    all_positions = [pos for sublist in top_10_gbest_positions for pos in sublist]
    return random.sample(all_positions, min(10, len(all_positions)))
...
while running and iteration < max_iteration:
    ...
    # After each iteration, store the current Gbest position
    current_top_10 = []
    for idx in np.argsort(Pbest_fitness)[:10]:  # Get indices of top 10 best
        current_top_10.append(Pbest_position[idx].copy())
    
    top_10_gbest_positions.append(current_top_10)
    save_top_10_gbest(iteration, current_top_10)
    ...
    # For next iteration, replace 10 random individuals with top 10 Gbest positions
    if iteration > 0:  # Start after first iteration
        random_top_10 = load_random_top_10()
        if random_top_10:
            replace_indices = random.sample(range(pop_size), 10)
            for i, idx in enumerate(replace_indices):
                if i < len(random_top_10):
                    P[idx] = random_top_10[i]
